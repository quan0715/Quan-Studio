...
cat > "${create_db_payload}" <<JSON
{
  ...
  "title": [
    { "type": "text", "text": { "content": "Quan Studio Articles" } }
  ],
  "description": [
    { "type": "text", "text": { "content": "Notion CMS database for Quan Studio backend sync." } }
  ]
}
JSON
...
if [ "${create_code}" != "200" ]; then
  echo "Failed to create database (HTTP ${create_code})"
  jq . "${create_db_resp}"
  exit 1
fi
...
if [ "${update_code}" != "200" ]; then
  echo "Database created but data source update failed (HTTP ${update_code})"
  jq . "${update_ds_resp}"
  exit 1
fi
...
echo "Blog data source id created: ${data_source_id}"
